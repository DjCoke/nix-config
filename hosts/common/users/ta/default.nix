...
let
  ...
  fullUserConfig = lib.optionalAttrs (!config.hostSpec.isMinimal) {
    ...
    home-manager.users.${config.hostSpec.username} = import (
      configLib.relativeToRoot "home/${config.hostSpec.username}/${config.networking.hostName}.nix"
    );
  };
in
{
    ...
    lib.recursiveUpdate fullUserConfig
      #this is the second argument to recursiveUpdate
      {
            ...
            plugins = [
              {
                ...
                src = configLib.relativeToRoot "home/${config.hostSpec.username}/common/core/zsh/p10k";
                file = "p10k.zsh";
              }
            ];
          };
        ...
        systemd.tmpfiles.rules =
          ...
          in
          [ "d /home/${config.hostSpec.username}/.ssh/sockets 0750 ${user} ${group} -" ];
