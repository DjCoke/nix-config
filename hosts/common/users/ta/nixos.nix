...
in
{
  ...
  users.users.${hostSpec.username} = {
    ...
    isNormalUser = true;
    hashedPasswordFile = sopsHashedPasswordFile; # Blank if sops is not working or running (i.e. isMinimal)
  ...
  # A proper root user required for borg and some other specific operations
  users.users.root = {
    shell = pkgs.zsh;
    hashedPasswordFile = config.users.users.${hostSpec.username}.hashedPasswordFile;
    hashedPassword = lib.mkForce config.users.users.${hostSpec.username}.password;
      ...
      plugins = [
        {
          ...
          src = configLib.relativeToRoot "home/${hostSpec.username}/common/core/zsh/p10k";
          file = "p10k.zsh";
        }
      ];
    };
